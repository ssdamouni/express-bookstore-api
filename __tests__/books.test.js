...
describe("PUT /books/:id", async function () {
  test("Updates a book by id", async function () {
    const response = await request(app)
        .put(`/books/${book_isbn}`)
        .send({
          amazon_url: "www.google/danish",
          author: "Steven Damouni",
          language: "Aramaic",
          pages: 800,
          publisher: "Penguin Publishing",
          title: "UPDATED BOOK",
  ...
  test(" 404 if can't find book doesn't exist", async function () {
    // delete book first
    await request(app)
